---
title: "Recluster epicardial lineage"
...
```{r}
epic <- subset(heart,subset = Broad_celltype == "Epicardial cells")
```
...
```{r}
epic.list <- SplitObject(epic, split.by = "biorep")
```

```{r}
for (i in 1:length(epic.list)) {
    epic.list[[i]] <- SCTransform(epic.list[[i]], verbose = FALSE)
}
```

```{r findanchors}
min(sapply(epic.list, ncol))
epic.anchors <- FindIntegrationAnchors(object.list = epic.list, dims=1:30,anchor.features = 3000,k.filter=92)
epic.integrated <- IntegrateData(anchorset = epic.anchors,dims=1:30)
```

# Perform clustering

```{r}
DefaultAssay(object = epic.integrated) <- "integrated"
```
...
epic.integrated <- FindClusters(epic.integrated, resolution = 0.1)
table(Idents(epic.integrated))
```
...
```{r tsne}
set.seed(10)
epic.integrated <- RunTSNE(epic.integrated, reduction = "pca", dims = 1:20)
```
...
```{r}
DimPlot(epic.integrated, reduction = "tsne", group.by = "orig.ident")
```
...
Idents(epic.integrated) <- epic.integrated$integrated_snn_res.0.1
```

```{r}
saveRDS(epic.integrated,file="./output/RDataObjects/epic-int-FYA-filtered.Rds")
...
write.csv(output,file="./output/MarkerAnalysis/Epicardial/Development/DE/MarkerAnalysis.csv")
```
